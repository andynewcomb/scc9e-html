...
# my $baseXMLfile = "scc9e-Part1Review.xml"; #-------------------------> identify digfir XML file here
# my $baseXMLfile = "scc9e-ch10.xml"; #-------------------------> identify digfir XML file here
# my $baseXMLfile = "scc9e-ch14.xml"; #-------------------------> identify digfir XML file here
# my $baseXMLfile = "scc9e-ch15.xml"; #-------------------------> identify digfir XML file here
# my $baseXMLfile = "scc9e-ch16.xml"; #-------------------------> identify digfir XML file here
# my $baseXMLfile = "scc9e-Part2Review.xml"; #-------------------------> identify digfir XML file here
my $baseXMLfile = "scc9e-Part3Review.xml"; #-------------------------> identify digfir XML file here
...
# figures (numbered in part-reviews)
if ($xml_contents =~ s/(<figure[^>]*block_type="FIGURE"[^>]*)(>.*?<phrase block_type="FG-N-ri">Figure (I|II|III|IV|V|VI)\.(\d+))/$1 data-filename="figure_$3_$4.html"$2/gs) {
  $chapter=$3;	
}
...
# exercises (part review)
if ($xml_contents =~ s/(<question[^>]*)(>\s*<p [^>]*>\s*<phrase block_type="CR-X-NL-N-ri">\s*(I|II|III|IV|V|VI)\.(\d+))/$1 block_type="exercise_$3_$4.html"$2/gs) {
  $chapter=$3;
}
...
# exercises (with icon in front - part review)
$xml_contents =~ s/(<question[^>]*)(>\s*<p [^>]*>\s*<image [^>]*>\s*<phrase block_type="CR-X-NL-N-ri">\s*(I|II|III|IV|V|VI)\.(\d+))/$1 block_type="exercise_$3_$4.html"$2/gs;
